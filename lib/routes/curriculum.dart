...
class _CurriculumState extends State<Curriculum> {
  ...
  //字体风格
  List<Color> fontColors = [Color(0xE52e383f), Colors.white];

  //风格
  List<String> styles = ["配色1", "配色2"];

  //控制跑马灯的高度
  double marqueeHeight=0;

  //存储跑马灯要显示的消息
  List<String> marqueeList=new List();
  ...
  void initState() {
    ...
    //获得当前周次
    weekNowInt=SpUtil.getInt(LocalShare.SERVER_WEEK);
  ...
  Card getHeader(int index) {
    if (index == 0) {
      return Card(
          ...
          child: Center(
            child: Text(DateTime.now().add(new Duration(days: ((weekNowInt-(_zcArray.indexOf(weekNow)))*-7))).month.toString()+"月",
          )));
    } else {
      //把今天对应的星期表头设置为红色
      if (DateTime.now().weekday == index) {
        return Card(
          ...
          child: Center(
            child: Text(
              dayOfWeek[index - 1] + "\n" + DateTime.now().add(new Duration(days: ((weekNowInt-_zcArray.indexOf(weekNow))*-7))).day.toString() + "日",
        ...
        return Card(
          ...
          child: Center(
            child: Text(
              dayOfWeek[index - 1] +
                  "\n" +
                  DateTime.now().add(new Duration(days: ((weekNowInt-_zcArray.indexOf(weekNow))*-7)))
                      .add(new Duration(days: (index - DateTime.now().weekday)))
                      .day
                      .toString()+
                  "日",
              textAlign: TextAlign.center,
    ...
    if (subjectList != null) {
      ...
      subjectList[classNum - 1].forEach((element) {
        ...
        if (element.weekDay == weekDay) {
          ...
          //不在这个周次,显示为灰色,当然还要判断当前位置是否已经显示了课程
          else if (w != weekDay && otherWeekVisible) {
            info = element.subjectName + "\n" + element.place;
      ...
      return Card(
        ...
        child: Center(
          child: InkWell(
            ...
            onTap:() => _showInfo(classNum, weekDay),
          ),
        ),
        // elevation: 10,
      );
  ...
  //获得跑马灯信息
  Future<void> _getMarMessage() async{
    //获得返回结果
    await _dio.get(Constant.MARQUEEMESSAGE).then((res) {
      //如果返回成功了
      if (res.statusCode == 200) {
        if (res.data["status"] == 200) {
          //处理数据
          String s=res.data["data"];
          marqueeList=s.split("&");
          for(int a=1;a<marqueeList.length;a++)
            {
              marqueeString=marqueeString+"                            "+marqueeList[a];
            }
        }
      }
    });
  ...
  //点击卡片弹出课程详细信息,classNum代表是第几大节,weekDay是星期几
  _showInfo(int classNum, int weekDay){
    //用于存储要展示详细信息的课程
    List<Subject> info=new List();
    //把要展示的课程添加到info中
    subjectList[classNum-1].forEach((element) {
      //在这个星期几上课
      if(element.weekDay==weekDay)
          ...
          children: <Widget>[
            Swiper(
              itemBuilder: (BuildContext context,int index)
              {
                return Card(
                  child: Container(
                    decoration: BoxDecoration(
                      image: DecorationImage(
                        image: AssetImage('assets/imgs/plane.png'),
                          fit: BoxFit.cover
                      )
                    ),
                    child: Column(
                      ...
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: <Widget>[
                        Text("课程名:"+info[index].subjectName),
                        Text("课程号:"+info[index].subjectNum),
                        Text("任课教师:"+info[index].teacherName),
                        Text("上课时间:"+info[index].classTime),
                      ],
                    ),
                  )
                );
              ...
              pagination: new SwiperPagination(
                ...
                margin: EdgeInsets.fromLTRB(0, MediaQuery.of(context).size.height/2, 0, 0)
              ),
            ...
            RaisedButton(
              child: Text("返回"),
              onPressed: () {
                Navigator.pop(context);//第二个参数是返回值
              },
            )
          ],
        )
